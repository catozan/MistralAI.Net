@echo off
echo Installing MistralAI.Net v1.0.2 Library...
echo.
...
echo.
echo Version 1.0.2 - API Endpoint URL Fix Release
echo - Fixed "no Route matched with those values" errors
echo - Corrected API endpoint URL construction
echo - Resolved routing issues with Mistral AI API
echo.
...
echo.
echo SUCCESS: MistralAI.Net.dll v1.0.2 copied to %projectPath%
...
if exist "%~dp0CHANGELOG.md" (
    start notepad.exe "%~dp0CHANGELOG.md"
) else (
...
echo.
echo v1.0.2 fixes the "no Route matched with those values" error
echo that was caused by incorrect API endpoint URL construction.
echo Your API calls should now work correctly!
echo.
pause
